...
def run_parser(parsers):
    ...
    if st.session_state['selected_parser']:
        ...
        default_parser = list(parsers.keys())[0] if parsers else None
            ...
            for uploaded_file in uploaded_files:
                ...
                try:
                    ...
                    if file_extension == ".pdf":
                        ...
                        with tempfile.TemporaryDirectory() as temp_dir_pdf:
                            pdf_path = os.path.join(temp_dir_pdf, uploaded_file.name)
                            with open(pdf_path, "wb") as f:
                                f.write(uploaded_file.read())
                            pages = convert_from_path(pdf_path)
                            for i, page in enumerate(pages):
                                page_filename = f"{os.path.splitext(uploaded_file.name)[0]}_page_{i+1}.jpg"
                                page_path = os.path.join(temp_dir_pdf, page_filename)
                    ...
                    if response.status_code == 200:
                        ...
                        if file_extension == ".pdf":
                            ...
                            with tempfile.TemporaryDirectory() as temp_dir_pdf:
                                ...
                                pdf_path = os.path.join(temp_dir_pdf, pdf_filename)
                                with open(pdf_path, 'wb') as f:
    ...
    if st.button("Run OCR"):
        ...
        def send_request(extra_accuracy):
            ...
            try:
                for file_path in image_paths + pdf_paths:
...
def display_ocr_results():
    ...
    if success_extra or success_no_extra:
        ...
        if success_extra and success_no_extra:
            comparison_results = generate_comparison_results(response_extra.json(), response_no_extra.json())
        ...
        if success_extra and success_no_extra:
            try:
                comparison_table = generate_comparison_df(response_extra.json(), response_no_extra.json(), comparison_results)
                ...
                st.error("Failed to parse JSON responses for comparison.")
                logging.error("Failed to parse JSON responses for comparison.")
        else:
        ...
        st.error("Cannot display OCR results as one or both responses are missing.")
